...
Lemma floor1_reverse_tech:
  forall r n,
    floor1 r = n -> IZR n < r <= IZR n + 1.
Proof.
  intros r n Hfloor1.
  unfold floor1 in Hfloor1.
  destruct floor1_ex as [v Hv] in Hfloor1.
  simpl in Hfloor1.
  rewrite Hfloor1 in Hv.
  apply Hv.
Qed.
...
Lemma floor1_lt_le:
  forall t1 t2, t1 < t2 -> (floor1 t1 <= floor1 t2)%Z.
Proof.
  intros t1 t2 Hlt.
  unfold floor1. 
  destruct (floor1_ex t1) as [t1_Z Ht1].
  destruct (floor1_ex t2) as [t2_Z Ht2].
  simpl. apply Zlt_succ_le. apply lt_IZR. rewrite succ_IZR.
  apply (Rlt_le_trans _ t2 _). apply (Rlt_trans _ t1 _).
  all: lra.
Qed.

Lemma floor1_positive:
  forall r, r > 0 -> (floor1 r >= 0)%Z.
Proof.
  intros r Hpos.
  unfold floor1.
  destruct (floor1_ex) as [r_Z HrZ].
  simpl. apply Z.le_ge.
  apply Zlt_succ_le.
  apply lt_IZR.
  rewrite succ_IZR.
  apply (Rlt_le_trans _ r _).
  all: lra.
Qed.

Lemma abs_floor_monotone:
  forall n r, r > 0 -> n = Z.abs_nat (floor1 r) -> 0 <= INR n < r.
Proof.
  intros n r Hr Hn.
  rewrite Hn.
  split.
  * apply pos_INR.
  * rewrite <- Hn.
    pose proof (floor1_positive _ Hr) as Hfloor_pos.
    apply Z.ge_le in Hfloor_pos.
    rewrite (Zabs2Nat.abs_nat_nonneg _ Hfloor_pos) in Hn.
    remember (floor1 r) as r_N.
    symmetry in Heqr_N.
    apply floor1_reverse_tech in Heqr_N.
    rewrite Hn.
    assert (Htechnical: forall z, (0 <= z)%Z -> INR (Z.to_nat z) = IZR z). {
      intros z Hz_pos.
      unfold Z.to_nat.
      destruct z.
      * reflexivity.
      * rewrite INR_IZR_INZ.
        ...
        reflexivity.
      * pose proof (Pos2Z.neg_is_nonpos p) as Hz_nonpos.
        pose proof (Z.le_antisymm _ _ Hz_nonpos Hz_pos) as Hz_value.
        rewrite Hz_value.
        reflexivity.
    }
    rewrite (Htechnical _ Hfloor_pos).
    apply Heqr_N.
Qed.

Lemma abs_floor_Sn: 
  forall n r, n = Z.abs_nat (floor1 r) -> r <= INR (S n).
Proof.
  intros n r Heq.
  rewrite Heq.
  remember (floor1 r) as r_Z.
  symmetry in Heqr_Z.
  apply floor1_reverse_tech in Heqr_Z.
  rewrite S_INR.
  rewrite INR_IZR_INZ.
  induction r_Z.
  * simpl; lra.
  * rewrite Zabs2Nat.inj_pos.
    rewrite positive_nat_Z.
    lra.
  * rewrite Zabs2Nat.inj_neg.  
    rewrite positive_nat_Z.
    rewrite IZR_NEG in Heqr_Z.
    apply (Rle_trans _ (- IZR (Z.pos p) + 1) _); try lra.
...
Lemma abs_floor_INR: 
  forall n, Z.abs_nat (floor1 (INR (S n))) = n.
Proof.
  intros n.
  rewrite (floor1_tech _ (Z.of_nat n)).
  * unfold Z.of_nat.
    destruct n.
    - reflexivity.
    - rewrite Zabs2Nat.inj_pos. 
      rewrite POrderedType.Positive_as_OT.of_nat_succ.
      apply Nat2Pos.id.
